...
<ParamField query="delivery" type="string">
  Delivery bucket. `instant` returns only items that are free of a Steam trade lock right now; `hold` returns only items still inside one, which cannot move until their `tradable_at`.
