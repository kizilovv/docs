...
<ParamField query="min_refund_percent" type="number">
  Only listings whose `refund_percent` is at least this (0–100). Listings with no published figure are **excluded**, not assumed.

  `?delivery=instant&min_refund_percent=100` is the pool a withdrawal flow wants: ships immediately, and a Steam reversal costs nobody anything.
</ParamField>
...
<ParamField query="available_after" type="string (ISO 8601)">
  Only return listings that entered the catalogue after this timestamp — the delta-poll parameter. Pair it with `sort=newest` and the usual response is an empty page, which is what makes frequent polling cheap instead of a full rescan every tick.
...
<ParamField query="sort" default="id" type="string">
...
<ResponseField name="items" type="Listing[]" required>
  ...
  <Expandable title="Listing object">
    ...
    <ResponseField name="delivery" type="string" required>
      Expected delivery mode: `instant` (bot, seconds to minutes), `up_to_12h` (a human seller on the source market must send the trade), or `hold` (inside a Steam trade lock until `tradable_at`).
    </ResponseField>

    <ResponseField name="refund_percent" type="number | null">
      How much of the purchase comes back if Valve reverses the Steam trade. This is a property of the market the listing came from, not a support policy, and it ranges from 0 to 100 across our sources.

      `null` means we have no published figure for that listing — treat it as unknown, never as 100. Filter with `min_refund_percent` to build a pool where every item is made whole.
    </ResponseField>

    <ResponseField name="listed_at" type="datetime | null">
      When this item entered the catalogue. **Not** the same as when the row was created: items return to the catalogue constantly — cancelled orders, expired trade offers, released holds — and a re-listed item is new to you even though it existed before. This is the value to feed back as `available_after`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
| 429 | `rate_limit_exceeded` | Over your key's per-minute limit (100/min by default). Back off for the number of seconds in the `Retry-After` header. |
