...
<ParamField query="delivery" type="string">
  Delivery bucket. `instant` returns only items that ship in seconds-to-minutes; `hold` returns only items still inside a Steam trade lock, which cannot move until their `tradable_at`.

  **Use `delivery=instant` for any flow that must deliver now** — a withdrawal, a bot payout, a user waiting on a screen. Without it the cheapest ask for an item can be a held row: genuinely buyable, but it will not arrive for hours. Any other value is a `400`, so a typo fails loudly instead of quietly widening your results.
</ParamField>
