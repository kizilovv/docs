...
- **Listings** — Live items available for purchase on the marketplace, including float, stickers, and ask price.
- **Prices** — Aggregated min-ask prices per skin, updated in real time. Also available as a bulk compressed snapshot.
- **Buying** — Place orders against your CSBoard balance using a trading-enabled key.
- **FX rates** — All prices in this API are **USD**. Use [`GET /v1/currency`](/api-reference/get-currency) to convert to a local currency.
