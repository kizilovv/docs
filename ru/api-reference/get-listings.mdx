...
<ResponseField name="items" type="Listing[]" required>
  ...
  <Expandable title="Listing object">
    ...
    <ResponseField name="listed_at" type="datetime | null">
      Когда предмет попал в каталог. **Не** то же самое, что момент создания записи: предметы возвращаются в каталог постоянно — отменённые заказы, истёкшие трейд-офферы, снятые холды — и перевыставленный предмет для вас новый, даже если существовал раньше. Именно это значение нужно передавать обратно в `available_after`.
    </ResponseField>
  </Expandable>
</ResponseField>
...
| 429 | `rate_limit_exceeded` | Превышен поминутный лимит вашего ключа (по умолчанию 100/мин). Подождите количество секунд, указанное в заголовке `Retry-After`. |
