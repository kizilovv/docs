...
| Параметр | Описание |
|-----------|-------------|
| `search` | Нечёткий поиск по подстроке в market hash name — подходит для поиска, но не позволяет изолировать имя, являющееся словесным подмножеством более длинного (например, `Spectrum Case` и `Spectrum 2 Case`) |
| `name` | **Точное** market hash name (регистр не учитывается) — точный способ найти один предмет. `name=Spectrum Case` вернёт только Spectrum Case. Имеет приоритет над `search` |
